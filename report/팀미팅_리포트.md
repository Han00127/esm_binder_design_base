...
# 15. L_map anchor 트랙 (2026-06-18 ~ 06-29)  ★ 이번 주

**브랜치:** `feat/map-anchored-loss` · **상세 작업로그:** `report/팀미팅_lmap_20260624.md`

## 15.0 한 줄 요약
distogram 설계에 **L_map anchor**(실험구조의 특정 접촉을 distogram에 직접 박는 손실)를 도입 → STEAP1 apex binder의 H3 8자리를 거의 완전히 새로 설계(**native 대비 7~8/8 변이**)했음에도 **8UCD 결합모드를 구조·상호작용 수준에서 재현**. L_inter 가중 강화로 **native를 넘는 설계 다수 확보**.

## 15.1 핵심 기술 — L_map anchor
**문제** — 기존 loose `inter_contact`("항원 20Å 이내면 만족")는 14Å에서 floating, gradient 소멸, 결합 핵심 잔기를 모름.

**아이디어** — ESMFold2가 만드는 **distogram(잔기쌍 거리분포)** 에, 8UCD가 알려준 **"어느 CDR잔기 ↔ 어느 항원잔기를 실제 몇 Å"** 를 직접 타깃으로 앵커링.
```
loose:  "항원 근처 어딘가"               → floating
L_map:  "H3 i번 ↔ 항원 V200을 8UCD 5.2Å로" → 명시적 앵커
```
...
| 구성 | 역할 | 손실 |
|---|---|---|
| 손가락(interface) | CDR 접촉잔기를 항원에 8UCD CA-CA 거리로 당김 | `L_inter` (정규화 MSE) |
| 손바닥(form) | 비접촉 H3는 loop 모양 유지 | `L_form` |
| 화학 | 잔기 화학성 보조 | comp/LM (저weight) |

**손실(개념)** — `L_inter = mean(((distogram기대거리[CDR_i,항원_j] − 8UCD거리_ij)/5.0)²)`, `L_struct = 2.0·L_inter + 0.5·L_form + …`. **mutable(설계가능) 잔기 접촉만 backward**.

**★ 숨은 핵심 — num_loops** — recycle 1회 distogram은 너무 거칠어 native 도킹조차 floating. **loops=3~5에서야 도킹 표현** → 설계 loops=3 채택(detach-first로 backward 메모리 불변).

> L_map = "실험구조 거리지도를 distogram에 직접 앵커링" + "recycle 3회로 도킹 표현". loose 제약의 floating을 구조적으로 해결.

## 15.2 이번 주 변경 + 결과
- **변경:** L_inter 가중 0.5→**2.0**(`--lambda-inter` 신규), trajectory 2→**30**(장기).
- **(a)** 평균 구조기반 ipSAE(vs 8UCD) **0.61→0.77**, 18개 중 **6개 native(0.756) 초과**, 상위 s3/s4 = 0.78mean·0.93max·**RMSD 2.4Å**. 30 trajectory `TRWGYYDFK…` 강한 수렴(s3·s4 동일서열=robust optimum).
- **(b) DockQ** — 항체-항원 0.65~0.69(medium~high). iRMSD↓(위치정확)+fnat↓(접촉 다름)=novel binder 특성.
- **(c) critic 교차검증** — s1은 base+Experimental critic 4종에서 0.67~0.91 재현 → **모델독립적=순환성 약함**. (critic OOM은 삼각곱셈 비청킹 문제로 규명→crop 해결.)
- **(d) full-H3 vs back-8** — H3 전부 설계 시 결합 상실(0.45~0.54, RMSD 4~10Å). **native 앵커(앞6) 유지가 결합의 핵심**.

## 15.3 ★ 중간 설계 분석 — 상호작용 보존 + 변이
설계영역(뒤8) native 대비 **7~8/8 변이**(거의 완전 novel). 8UCD 42개 side-chain 상호작용 측정(`runs/check_interaction.py`, 15-fold consensus, 5.5Å):

| 설계 | 변이/8 | 접촉유지 | 고정부 | 설계부 | key앵커(V200/Q201) | ipSAE |
|---|---|---|---|---|---|---|
| REF(native) | 0/8 | 93% | 93% | — | 18/19 | 0.756 |
| s16 | 8/8 | 81% | 85% | **67%** | 15/19 | 0.763 |
| s6 | 8/8 | **83%** | 91% | 56% | 15/19 | 0.758 |
| s7/s11/s5/s12 | 7~8/8 | 79~83% | 85~91% | 56% | 14~15/19 | 0.75~0.76 |
| s0/s15 | 8/8 | 0% | 0% | 0% | 0/19 | floated |

→ **18개 중 11개가 상호작용 ≥70% 유지.**

**해석:** ① **서열-결합 분리** — 7~8/8 변이인데 70~83% 유지 = 결합은 서열보존이 아닌 **L_map 앵커링 구조**에서 나옴. ② 고정부 82~91%(도킹 정확) vs 설계부 44~67%(특이접촉 절반, DockQ 낮은 fnat과 일치). ③ 공통 치환 **G→D,T→F,R→K,V→Y** 독립 수렴=결합유리 후보. ④ floater(s0/s15) 0% = ipSAE/RMSD와 교차일치.

## 15.4 종합 best · 한계 · 다음
- **best:** s16(설계부 67%+8/8변이) / s6(접촉 83%+key 15/19) — 완전 novel 서열로 near-native 결합.
- **한계:** 8UCD 기준 평가(결합모드 *재현* 증명, 절대 binder 미증명) / 모델PAE 저확신 / 설계부 특이접촉 절반 손실 → 친화도 검증 필요.
- **다음:** traj30 완주(18/30 진행 중) 후 top critic+DockQ 정밀검증 / 공통치환 기여 분석 / GPU 가용 시 3세트 병렬(7.5→2.5일).
